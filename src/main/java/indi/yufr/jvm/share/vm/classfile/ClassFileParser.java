...
public class ClassFileParser {
    ...
    public static InstanceKlass parseClassFile(byte[] content) {
        ...
        for (int i = 0; i < klass.getAttributeLength(); i++) {
            ...
            if (attrName.equals("SourceFile")) {
                ...
                index = parseRuntimeInvisibleAnnotations(content, index, klass);
            } else if (attrName.equals("RuntimeVisibleAnnotations")) {
                index = parseRuntimeVisibleAnnotations(content, index, klass);
            } else {
    ...
    private static int parseRuntimeVisibleAnnotations(byte[] content, int index, InstanceKlass klass) {
        byte[] u2Arr = new byte[2];
        byte[] u4Arr = new byte[4];
    ...
    private static int parseRuntimeInvisibleAnnotations(byte[] content, int index, InstanceKlass klass) {
        byte[] u2Arr = new byte[2];
        byte[] u4Arr = new byte[4];
